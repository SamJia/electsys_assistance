根据自己情况，修改好配置文件config.txt(如未找到该文件，则在与electsys_assistance.exe同目录下创建即可)，运行electsys_assistance.exe文件即可。程序会自动打开浏览器并进行自动化抢课。

关于配置文件格式：
1.前两行分别为Jaccount账号和密码，程序会自动使用Jaccount登录，请在程序登陆时使用英文输入法，否则可能出现无法正确填写登录信息的情况。
